...
We could also create a helper function for the if-else statements (shown in the image below) that checks for array types. This would further reduce the complexity of the append method. However, this would mostly just move a large part of the logic to another method rather than really simplifying the approach. While that would lower the cyclomatic complexity of the append method it wouldn't provide a meaningful reduction in the overall complexity of the class. Since the array comparisons must still be handled explicitly. Given this the extra method is unnecessary as it does not significantly improve readability or maintainability.
